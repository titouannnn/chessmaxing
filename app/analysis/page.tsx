...
const CLASSIFICATION_ICONS: Record<MoveClassification, { icon: string, color: string, label: string }> = {
  ...
  imprecision: { 
    icon: '<path d="M11 15v2h2v-2h-2Zm0-8v6h2V7h-2Zm5.17 8a2 2 0 1 1-2.83 2.83 2 2 0 0 1 2.83-2.83ZM13 7h2v6h-2V7Z" fill="white"/>', 
    color: '#fbc02d', 
    label: '?!' 
  },
  erreur: { 
...
const classifyMove = (
  ...
  const wpBefore = currentEval.wp;
  const wpAfter = nextEval.wp;
  
  const chess = new Chess(fens[moveIdx] || START_FEN);
...
export default function AnalysisPage() {
  ...
  const loadPgn = useCallback((pgnStr: string) => {
    ...
    try {
      ...
      let success = false;
      try { chess.loadPgn(cleanPgn); success = true; } catch (e) {}
      if (!success) {
        const pgnSimple = cleanPgn.replace(/\{.*?\}/gs, "").replace(/\(.*\)/gs, "").replace(/\d+\.\.\./g, "").trim();
        try { chess.loadPgn(pgnSimple); success = true; } catch (e) {}
      }
      if (!success) {
        const moveText = cleanPgn.includes("\n\n") ? cleanPgn.split("\n\n").pop()! : cleanPgn;
        const moves = moveText.replace(/\{.*?\}/gs, "").replace(/\d+\./g, " ").split(/\s+/).filter(m => m && !m.includes(".") && !m.includes("-") || /^[a-hNKQRB][a-h1-8x+#=]*$/.test(m) || m === "O-O" || m === "O-O-O");
        chess.reset();
        for (const m of moves) { try { chess.move(m); } catch (e) { break; } }
      ...
      if (!headers.White || headers.White === "?") {
        const whiteMatch = pgnStr.match(/\[White\s+"(.*?)"\]/), blackMatch = pgnStr.match(/\[Black\s+"(.*?)"\]/);
        const wElo = pgnStr.match(/\[WhiteElo\s+"(.*?)"\]/), bElo = pgnStr.match(/\[BlackElo\s+"(.*?)"\]/);
        if (whiteMatch) headers.White = whiteMatch[1];
        if (blackMatch) headers.Black = blackMatch[1];
        if (wElo) headers.WhiteElo = wElo[1];
        if (bElo) headers.BlackElo = bElo[1];
      }
      ...
      setEngineMode('idle');
      setPgnInput(pgnStr);
    } catch (e) { console.error("Critical PGN Error", e); }
  }, [username]);
  ...
  processNextReviewMoveRef.current = useCallback(() => {
    ...
    if (chess.isGameOver()) {
      ...
      reviewResultsRef.current[idx] = { moveIndex: idx - 1, evaluation: winProb * 10, cp: winProb * 1000, playedMove: playedMoveUci, wp: winProb > 0 ? 1 : (winProb < 0 ? 0 : 0.5) };
    ...
    onMessageRef.current = (e) => {
      ...
      if (mode === 'review' && isReviewActiveRef.current) {
        ...
        if (line.startsWith("info depth")) {
          const scoreMatch = line.match(/\bscore (cp|mate) (-?\d+)/), pvMatch = line.match(/\bpv\s+([a-h][1-8][a-h][1-8][qrbn]?)/);
          const multipvMatch = line.match(/\bmultipv (\d+)/);
          const multipv = multipvMatch ? parseInt(multipvMatch[1], 10) : 1;
          if (scoreMatch) {
            const currentIdx = reviewCurrentIdx.current;
            const turn = new Chess(reviewQueueRef.current[currentIdx]).turn();
            ...
            const existingEval = reviewResultsRef.current[currentIdx] || { moveIndex: currentIdx - 1, evaluation: getWinningChance(val) * 10 };
            if (multipv === 1) {
              let playedMoveUci: string | undefined = undefined;
              const playedMoveSan = history[currentIdx];
              if (playedMoveSan) { try { const tempChess = new Chess(reviewQueueRef.current[currentIdx]); const m = tempChess.move(playedMoveSan); playedMoveUci = m.from + m.to; } catch (e) {} }
              reviewResultsRef.current[currentIdx] = { ...existingEval, mate: mate !== undefined ? mate * multiplier : undefined, cp: val, bestMove: pvMatch ? pvMatch[1] : undefined, playedMove: playedMoveUci, wp: wp };
            } else if (multipv === 2) {
              reviewResultsRef.current[currentIdx] = { ...existingEval, wp2: wp };
            }
          }
        }
      }
            ...
            if (depth >= 14 && isVariation) {
              setVariationEvaluations(prev => {
                ...
                if (multipvIdx === 1) {
                  let playedMoveUci: string | undefined = undefined;
                  const playedMoveSan = history[currentIdx];
                  if (playedMoveSan) { try { const tChess = new Chess(fen); const m = tChess.move(playedMoveSan); playedMoveUci = m.from + m.to; } catch (e) {} }
                  updated.cp = val; updated.mate = rawMate !== undefined ? rawMate * multiplier : undefined; updated.bestMove = uciMoves[0]; updated.playedMove = playedMoveUci; updated.wp = wp; updated.evaluation = getWinningChance(val) * 10;
                } else if (multipvIdx === 2) { updated.wp2 = wp; }
                return prev.some(e => e.moveIndex === currentIdx) ? prev.map(e => e.moveIndex === currentIdx ? updated : e) : [...prev, updated];
              });
            }
          }
  ...
  useEffect(() => {
    const onPgnReceived = (pgn: string) => {
      ...
      setPgnInput(pgn); loadPgn(pgn);
      window.postMessage({ type: "PGN_RECEIVED" }, "*");
    };
    const handleMessage = (event: MessageEvent) => { if (event.data?.type === "FROM_EXTENSION" && event.data?.pgn) onPgnReceived(event.data.pgn); };
    window.addEventListener("message", handleMessage);
    window.postMessage({ type: "GET_PGN_FROM_EXTENSION" }, "*");
    const timeout = setTimeout(() => { window.postMessage({ type: "GET_PGN_FROM_EXTENSION" }, "*"); }, 1000);
      ...
      if (container) {
        requestAnimationFrame(() => {
          if (activeMoveRef.current && container) {
            const el = activeMoveRef.current, elTop = el.offsetTop, elHeight = el.offsetHeight, containerHeight = container.clientHeight;
  ...
  useEffect(() => {
    const board = boardContainerRef.current, graphBox = graphContainerRef.current;
  ...
  const autoShapes = useMemo(() => {
    const shapes: any[] = [];
    if (showPlayedArrows && currentMoveIndex < history.length - 1) {
      try { const t = new Chess(fen); const m = t.move(history[currentMoveIndex + 1]); shapes.push({ orig: m.from as Key, dest: m.to as Key, brush: "green" }); } catch(e) {}
    }
    if (showEngineArrows) {
      if (engineMode === 'analysis') {
        const best = engineInfo.lines.find(l => l.id === 1);
        if (best && engineInfo.depth > 0 && best.pv.length > 0) { const move = best.pv[0], orig = move.substring(0, 2) as Key, dest = move.substring(2, 4) as Key; shapes.push({ orig, dest, brush: "blue" }); }
        ...
        if (currentEval?.bestMove) { const orig = currentEval.bestMove.substring(0, 2) as Key, dest = currentEval.bestMove.substring(2, 4) as Key; shapes.push({ orig, dest, brush: "blue" }); }
      }
    }
    if (activeTab === 'bilan' && (gameEvaluations.length > 0 || variationEvaluations.length > 0) && currentMoveIndex >= 0 && !isReviewActiveRef.current) {
      const fens = reviewQueueRef.current, moveEval = classifications[currentMoveIndex];
      if (moveEval) {
        const lastMoveSan = history[currentMoveIndex];
        try { const tempChess = new Chess(fens[currentMoveIndex] || START_FEN); const m = tempChess.move(lastMoveSan); shapes.push({ orig: m.to as Key, customSvg: { html: generateBadgeSvg(moveEval.classification), center: 'orig' } }); } catch(e) {}
      }
    }
    return shapes;
  }, [engineMode, engineInfo, currentMoveIndex, showEngineArrows, showPlayedArrows, activeTab, gameEvaluations, variationEvaluations, classifications, history, fen]);
  ...
  const renderReport = () => {
    ...
    const counts = { w: categories.reduce((acc, cat) => ({ ...acc, [cat]: 0 }), {} as Record<MoveClassification, number>), b: categories.reduce((acc, cat) => ({ ...acc, [cat]: 0 }), {} as Record<MoveClassification, number>) };
    const accuracies = { w: [] as number[], b: [] as number[] };
    mainClassifications.forEach((c, i) => { const color = i % 2 === 0 ? 'w' : 'b'; counts[color][c.classification]++; if (c.classification !== 'theorique') accuracies[color].push(c.accuracy); });
    const avgAcc = { w: accuracies.w.length > 0 ? accuracies.w.reduce((a, b) => a + b, 0) / accuracies.w.length : 100, b: accuracies.b.length > 0 ? accuracies.b.reduce((a, b) => a + b, 0) / accuracies.b.length : 100 };
    const getBadgeColor = (c: MoveClassification) => { switch(c) { case 'incroyable': return 'bg-cyan-500 text-white'; case 'excellent': return 'bg-blue-500 text-white'; case 'meilleur': return 'bg-green-500 text-white'; case 'tres_bien': return 'bg-green-400 text-white'; case 'bon': return 'bg-emerald-400 text-white'; case 'theorique': return 'bg-stone-500 text-white'; case 'imprecision': return 'bg-yellow-500 text-white'; case 'erreur': return 'bg-orange-500 text-white'; case 'gaffe': return 'bg-red-500 text-white'; } };
    const getLabel = (c: MoveClassification) => { switch(c) { case 'incroyable': return 'Incroyable !!'; case 'excellent': return 'Excellent'; case 'meilleur': return 'Meilleur'; case 'tres_bien': return 'Très Bien'; case 'bon': return 'Bon'; case 'theorique': return 'Théorique'; case 'imprecision': return 'Imprécision'; case 'erreur': return 'Erreur'; case 'gaffe': return 'Gaffe'; } };
    ...
    return (
      <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-4 flex flex-col gap-4 animate-in fade-in flex-1 min-h-0 overflow-hidden shadow-xl">
        ...
        <div className="grid grid-cols-2 gap-4 shrink-0">
          <div className="bg-white/[0.03] rounded-lg p-3 border border-white/5 flex flex-col items-center"><span className="text-[10px] font-black text-stone-500 uppercase mb-1">Précision Blancs</span><div className="text-2xl font-black text-white">{avgAcc.w.toFixed(1)}%</div></div>
          <div className="bg-white/[0.03] rounded-lg p-3 border border-white/5 flex flex-col items-center"><span className="text-[10px] font-black text-stone-500 uppercase mb-1">Précision Noirs</span><div className="text-2xl font-black text-white">{avgAcc.b.toFixed(1)}%</div></div>
        </div>
        <div className="flex flex-col gap-2 shrink-0">
          <div className="grid grid-cols-3 text-center border-b border-white/5 pb-2"><span className="text-[10px] font-black text-stone-500 uppercase">Blancs</span><span className="text-[10px] font-black text-stone-600 uppercase">Catégorie</span><span className="text-[10px] font-black text-stone-500 uppercase">Noirs</span></div>
  ...
  return (
    <main className="max-w-[1400px] mx-auto p-4 md:p-8 space-y-6 text-white min-h-screen bg-[#0a0a0a]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[60%_40%] gap-8 items-stretch relative min-h-[600px]">
        ...
        <div className="relative min-w-0 min-h-[600px] flex-grow flex flex-col">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex flex-col h-full min-h-0">
              ...
              <TabsContent value="bilan" className="flex-1 hidden data-[state=active]:flex flex-col gap-4 mt-6 animate-in fade-in duration-300 min-h-0">
                ...
                {engineMode !== 'review' && gameEvaluations.length === 0 && (
                  <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-4 flex flex-col flex-1 min-h-0 shrink-0 overflow-hidden">
                    <div className="flex items-center justify-between mb-6 shrink-0"><h2 className="text-xs font-black uppercase tracking-[0.2em] text-stone-500">Navigation Partie</h2></div>
              ...
              <TabsContent value="analyse" className="flex-1 hidden data-[state=active]:flex flex-col gap-4 mt-6 animate-in fade-in duration-300 min-h-0">
                ...
                <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-4 flex flex-col flex-1 min-h-0 overflow-hidden">
                  <div className="flex items-center justify-between mb-6 shrink-0"><h2 className="text-xs font-black uppercase tracking-[0.2em] text-stone-500">Historique</h2></div>
